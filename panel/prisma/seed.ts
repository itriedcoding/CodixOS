...
async function main() {
  ...
  console.log("Nodes created:", node1.name, node2.name);
  ...
  console.log("Servers created:", server1.name, server2.name, server3.name, server4.name, server5.name, server6.name, server7.name, server8.name);

  await prisma.activityLog.createMany({
    data: [
      {
        user_id: admin.id,
        event: 'Server "Survival MC" was created',
        ip: "192.168.1.1",
        data: { serverId: server1.id },
      },
      {
        user_id: admin.id,
        event: 'Server "Rust EU Official" was started',
        ip: "192.168.1.1",
        data: { serverId: server3.id },
      },
      {
        user_id: admin.id,
        event: 'Node "US-East-1" was registered',
        ip: "192.168.1.1",
        data: { nodeId: node1.id },
      },
      {
        user_id: admin.id,
        event: 'Backup created for "CS2 Competitive"',
        ip: "192.168.1.1",
        data: { serverId: server4.id },
      },
      {
        user_id: admin.id,
        event: 'Server "ARK Island" was updated',
        ip: "192.168.1.1",
        data: { serverId: server5.id },
      },
    ],
  });
